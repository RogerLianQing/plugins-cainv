...
function pms_cron_process_member_subscriptions_payments() {
    ...
    foreach( $subscriptions as $subscription ) {
		...
		$inst =  get_field('discount', 'user_' .  $subscription->user_id);
		$multiplier = (100 - $inst) / 100;
		if($inst >= 100){
			$multiplier = 0;
		}
		$total = $multiplier *  $subscription->billing_amount;
		update_field('discount', 0 ,  'user_' . $subscription->user_id);
